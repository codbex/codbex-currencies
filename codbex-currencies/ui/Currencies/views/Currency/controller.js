...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'codbex-launchpad.Currencies.Currency.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('codbex-launchpad.Currencies.Currency.refresh', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	var api = '../../../../../../../../services/v4/js/codbex-currencies/api/Currencies/Currency.js';
	...
	$scope.update = function() {
		if ($scope.entityForm.$valid) {
			$http.put(api + '/' + $scope.entity.Code, JSON.stringify($scope.entity))
	...
	$scope.delete = function() {
		$http.delete(api + '/' + $scope.entity.Code)
		.then(function(data) {
			$scope.loadPage($scope.dataPage);
			toggleEntityModal();
			$messageHub.messageEntityModified();
