angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope',  'Extensions', 'messageHub', 'entityApi', function ($scope,  Extensions, messageHub, entityApi) {
		...
		Extensions.get('dialogWindow', 'codbex-currencies-custom-action').then(function (response) {
			$scope.entityActions = response.filter(e => e.perspective === "Currencies" && e.view === "Currency" && e.type === "entity");
		});
		...
		$scope.create = function () {
			entityApi.create($scope.entity).then(function (response) {
				if (response.status != 201) {
					messageHub.showAlertError("Currency", `Unable to create Currency: '${response.message}'`);
					return;
				}
		...
		$scope.update = function () {
			entityApi.update($scope.entity.Id, $scope.entity).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("Currency", `Unable to update Currency: '${response.message}'`);
					return;
				}
